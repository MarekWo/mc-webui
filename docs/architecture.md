...
| POST | `/api/repeaters/<pk>/regions` | One region action (`{action: add\|remove\|move\|allow_flood\|deny_flood\|set_home\|set_default\|clear_default\|save, name?, parent?}`); admin only |
